...
export const SUGGESTIONS: Suggestion[] = [
  ...
  { label: "Tem em estoque? 🔎", prompt: "Como sei se um produto está disponível?" },
...
export function buildSystemPrompt(products: Product[]): string {
  ...
  return `Você é ${assistant.name}, ${assistant.role} da ${market.name}, um mercado de bairro acolhedor e de confiança.
...
- Respostas curtas: no máximo 3–4 linhas. Preços sempre em reais com vírgula (ex.: R$ 4,89).
...
CATÁLOGO ATUAL DA ${market.name.toUpperCase()} (use exatamente estes dados):
${catalog}
