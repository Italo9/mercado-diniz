...
const DISC = { sm: "w-9 h-9", md: "w-11 h-11", lg: "w-14 h-14" }
const CART = { sm: "w-5 h-5", md: "w-6 h-6", lg: "w-7 h-7" }
const WORDMARK = { sm: "text-lg", md: "text-2xl", lg: "text-3xl" }
const TAGLINE = { sm: "text-[9px]", md: "text-[10px]", lg: "text-xs" }
...
export function Logo({
  ...
  return (
    <span className={clsx("flex items-center gap-2.5 group", className)}>
      ...
      >
        <ShoppingCart className={clsx("text-sage-800", CART[size])} strokeWidth={2.4} aria-hidden="true" />
      </span>
      ...
      <span className="flex flex-col leading-none">
        ...
        {showTagline && tagline && (
          <span
            className={clsx(
              "font-body font-semibold uppercase leading-none mt-0.5 tracking-[0.18em]",
              TAGLINE[size],
